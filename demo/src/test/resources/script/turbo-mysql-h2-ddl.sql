...
) ENGINE=InnoDB DEFAULT CHARSET=utf8 ROW_FORMAT=COMPACT COMMENT='父子流程实例映射表';
...
) ENGINE=InnoDB ROW_FORMAT=COMPACT COMMENT='实例数据表';

DROP TABLE IF EXISTS `ei_node_instance_parallel`;
CREATE TABLE IF NOT EXISTS `ei_node_instance_parallel` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '自增主键',
  `execute_id` varchar(255) DEFAULT NULL COMMENT '执行id',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB COMMENT='节点执行平行表';
...
CREATE TABLE IF NOT EXISTS `ei_node_instance_log_parallel` (
  ...
  `execute_id` varchar(255) DEFAULT NULL COMMENT '执行id',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB COMMENT='节点执行记录平行表';
