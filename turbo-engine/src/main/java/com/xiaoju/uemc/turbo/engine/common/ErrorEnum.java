...
public enum ErrorEnum {
    ...
    //3000~3999 流程定义错误
    DEFINITION_INSERT_INVALID(3001, "数据库插入失败"), //DEFINITION_INSERT_INVALID(3001, "Database insert failed"),
    DEFINITION_UPDATE_INVALID(3002, "数据库更新失败"), //DEFINITION_UPDATE_INVALID(3002, "Database update failed"),

    MODEL_EMPTY(3201, "模型为空"),//MODEL_EMPTY(3201, "Empty model"),
    MODEL_MUST_ONE_START_NODE(3202, "流程必须有且仅有一个开始节点"),//MODEL_NO_START_NODE(3202, "No start node"),
    ELEMENT_KEY_NOT_UNIQUE(3203, "流程元素key必须唯一"),//ELEMENT_KEY_NOT_UNIQUE(3203, "Element key not unique"),
    MODEL_NO_END_NODE(3204, "流程至少需要有一个结束节点"),//MODEL_NO_END_NODE(3204, "No end node"),
    MODEL_NOT_UNICOM(3205, "该流程从开始节点不能到底每一个节点"),//MODEL_NOT_UNICOM(3205, "No unicom"),
    ...
    TERMINATE_CANNOT_COMMIT(4003, "流程实例已终止, 无法继续提交"),//TERMINATE_FLOW_COMMIT(4003, "Terminate flow commit"),
    FLOW_INSTANCE_CANNOT_ROLLBACK(4004, "非执行中流程实例, 无法撤销"),//FLOW_INSTANCE_ROLLBACK(4004, "Non-running flow instance rollback"),
    NO_NODE_TO_ROLLBACK(4005, "该实例没有可撤销的节点"),//NO_NODE_TO_ROLLBACK(4005, "No node to rollback"),
    NO_USER_TASK_TO_ROLLBACK(4006, "已完成第一个用户节点的撤销, 无法继续撤销"),//NO_USER_TASK_TO_ROLLBACK(4006, "No user task to rollback"),
    GET_FLOW_DEPLOYMENT_FAILED(4007, "获取不到流程部署信息"),//GET_FLOW_DEPLOYMENT_FAILED(4007, "Get flow deployment failed"),
    GET_FLOW_INSTANCE_FAILED(4008, "获取不到流程实例信息"),//GET_FLOW_INSTANCE_FAILED(4008, "Get flow instance failed"),
    GET_NODE_FAILED(4009, "获取不到待处理的节点"),//GET_NODE_FAILED(4009, "Get current node failed"),
    GET_NODE_INSTANCE_FAILED(4010, "获取不到节点实例信息"),//GET_NODE_INSTANCE_FAILED(4010, "Get node instance failed"),
    GET_INSTANCE_DATA_FAILED(4011, "获取不到实例数据信息"),//GET_INSTANCE_DATA_FAILED(4011, "Get instance data failed"),
    ...
    SAVE_FLOW_INSTANCE_FAILED(4016, "保存流程实例失败"),//SAVE_FLOW_INSTANCE_FAILED(4016, "Save flow instance failed"),
    SAVE_INSTANCE_DATA_FAILED(4017, "保存实例数据失败"),//SAVE_INSTANCE_DATA_FAILED(4017, "Save instance data failed"),
